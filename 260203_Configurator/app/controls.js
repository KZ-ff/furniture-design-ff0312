...
export function createControls({ camera, renderer, scene, ui, onSelect }) {
  ...
  transform.addEventListener("dragging-changed", (e) => {
    ...
    // Hand cursor while dragging an object
    document.body.classList.toggle("dragging-obj", e.value);
    if (e.value) document.body.classList.remove("dragging-cam");
  });

  // Camera cursor while orbiting/panning the viewport
  orbit.addEventListener("start", () => document.body.classList.add("dragging-cam"));
  orbit.addEventListener("end",   () => document.body.classList.remove("dragging-cam"));
  ...
  window.addEventListener("click", (event) => {
    if (event.target.closest(".panel, .sidebar, #right-panel")) return;

    const vpArea = document.getElementById("viewport-area");
    const rect = vpArea
      ? vpArea.getBoundingClientRect()
